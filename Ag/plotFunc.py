...
def get_coords_from_csv_xy(file_name):
    import csv
    coords = []
...
def get_coords_from_csv_data(file_name):
    import csv
    coords = []
    with open(f'{file_name}.csv', 'r', encoding='UTF-8') as csvFile:
        reader = csv.reader(csvFile)
...
class PieChartScene(Scene):
    def construct(self):
        ...
        pie_chart = PieChart(legend_BUFF=MED_LARGE_BUFF)
        ...
        self.play(
            LaggedStartMap(ShowCreation,VGroup(*[obj for obj in pc_arcs]),lag_ratio=1),
            LaggedStartMap(FadeIn,VGroup(*[obj for obj in pc_legends],shift=UP),lag_ratio=1,),
            run_time=5,
        )
