...
class TimeFormatCountdown(Scene):
    def construct(self):
        ...
        countdown_number = DecimalNumber(
            total_seconds, num_decimal_places=0, font_size=48
        )
        ...
        # 执行倒计时动画
        self.play(
            ChangeDecimalToValue(countdown_number, 0), run_time=10, rate_func=linear
        )
        self.wait(1)
...
class 齐奥尔科夫斯基火箭方程(Scene):
    def construct(self):
        ...
        VGroup(tex1, tex2).arrange(DOWN, buff=0.4, aligned_edge=LEFT).scale(1.5).shift(
            UP * 0.22
        )
...
class 洛伦茨吸引子(Scene):
    def construct(self):
        tex = TexText(
            ...
            "$$\\frac{d x}{d t}=x y-\\beta z$$",
            font="SimSun",
        )

        text1 = TexText(
            "$$\\text{其中，}x \\text{：对流强度，}，y \\text{：水平温差，}，z \\text{：垂直温度剖面畸变程度；}$$",
            font_size=30,
            font="SimSun",
        )

        text2 = TexText(
            "$$\\sigma \\text{：普朗特数}，\\rho \\text{：瑞利数}，\\beta \\text{：几何因子。}$$",
            font_size=30,
            font="SimSun",
        )

        tex.arrange(DOWN, buff=0.5, aligned_edge=LEFT)
        tex.scale(1).shift(UP * 0.618)
        text1.scale(0.8).next_to(tex, DOWN, buff=0.6, aligned_edge=LEFT)
        text2.scale(0.8).next_to(text1, DOWN, buff=0.25, aligned_edge=LEFT)

        self.play(Write(tex), run_time=1)
        self.play(FadeIn(text1, shift=UP), run_time=1)
        self.wait()
        self.play(FadeIn(text2, shift=UP), run_time=1)
...
if __name__ == "__main__":
    ...
    system("manimgl {} ClosedUniverse -o".format(__file__))
