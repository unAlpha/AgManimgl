...
class TimeCosts(Scene):
    ...
    def construct(self):
        ...
        self.play(
                LaggedStartMap(FadeIn,pvg,scale=0.9,shift=UP,lag_ratio=0.1),
                run_time=1
            )
        self.wait()
...
class ChatGPTCosts(Scene):
    def construct(self):
        ...
        pvg = VGroup()
        for p in ptxt:
            pvg.add(TexText(p,font_size=60,))
        pvg.arrange(DOWN)
...
class AutoGPTCosts(Scene):
    def construct(self):
        ...
        # bg = FullScreenRectangle(fill_color=["#032348","#46246d","#31580a","#852211"])
        # self.add(bg)    
        self.play(FadeIn(pvg[0],scale=0.5,shift=RIGHT),)
        self.wait()
        for i in range(len(pvg)-1):
            self.play(
                GrowArrow(Arrvg[i]),
                FadeIn(pvg[i+1],scale=0.5))
            self.wait()
        self.wait()    
