...
class WordsChange1(Scene):
    def construct(self):
        ...
        for i in range(len(Q)):
            question = TexText(Q[i], isolate=Q_words, font_size=28, alignment="\\raggedright")
            Vg_q.add(question)
        self.add(Vg_q[0])
        ...
        self.play(
            black_to_white_transform, FadeIn(seed_part)
        )
...
class PictureShow1(Scene):
    ...
    Anim_words = [["black", "white", "--v 5"],["黑","白", "--v 5"],["--seed 4015514435"]]
    Coat = [
        ["黑色外套","AgPics/外国人黑外套",],
        ["白色外套","AgPics/外国人白外套",]
        ]
    ...
    def construct(self):
        ...
        for i in range(2):
            prompt = Text("关键词",color=RED,font = '阿里巴巴普惠体 2.0',font_size=40,)
            question = TexText(self.Q[i],isolate=self.Anim_words[0], font_size=self.f,alignment="\\raggedright")
            pq = VGroup(prompt,question).arrange(DOWN,aligned_edge=LEFT,buff=0.25)

            translate = Text("翻译",color=GREY,font = '阿里巴巴普惠体 2.0',font_size=40,)
            answer= TexText(self.Ans[i],isolate=self.Anim_words[1],color=YELLOW, font_size=self.f, alignment="\\raggedright")
            at = VGroup(translate,answer).arrange(DOWN,aligned_edge=LEFT,buff=0.25)
            
            VGpqat.add(VGroup(pq,at).arrange(DOWN,aligned_edge=LEFT,buff=0.5).center().to_edge(LEFT,buff=1).shift(UP*0.2))
        
        for i in range(len(self.Coat)):    
            coattxt = Text(self.Coat[i][0],color=WHITE,font = '阿里巴巴普惠体 2.0',font_size=40,)
            pic_vg = Group()
        ...
        black_to_white_transform1 = ReplacementTransform(
            ...
            VGpqat[1][0][1].get_part_by_tex(self.Anim_words[0][1]).move_to(VGpqat[0][0][1].get_part_by_tex(self.Anim_words[0][0]))
        )
        seed_part1 = TexText(self.Anim_words[2][0], font_size=self.f)
        seed_part1.next_to(VGpqat[0][0][1].get_part_by_tex(self.Anim_words[0][2]), RIGHT, buff=0.13)

        black_to_white_transform2 = ReplacementTransform(
            VGpqat[0][1][1].get_part_by_tex(self.Anim_words[1][0]),
            VGpqat[1][1][1].get_part_by_tex(self.Anim_words[1][1]).move_to(VGpqat[0][1][1].get_part_by_tex(self.Anim_words[1][0]))
        )
        seed_part2 = TexText(self.Anim_words[2][0],color=YELLOW, font_size=self.f)
        seed_part2.next_to(VGpqat[0][1][1].get_part_by_tex(self.Anim_words[1][2]), RIGHT, buff=0.13)        
        bg = FullScreenRectangle(fill_color=["#032348","#46246d","#31580a","#852211"])
        self.add(bg)
        self.wait(1)
        self.play(
        ...
        self.wait(0.2)
        self.play(FadeIn(VGcg[0],shift=RIGHT,))
        self.play(
            FadeIn(VGpqat[0][1][0]),
            ShowIncreasingSubsets(VGpqat[0][1][1]),
            run_time=3,
        ...
        for j in range(1,len(self.Q)):
            self.play(
                ...
                FadeOut(VGcg[j-1],shift=LEFT,),
                FadeIn(vgjcopy,shift=RIGHT,)
            )
            self.wait(3)
        ...
        self.play(
            LaggedStart(
                ...
                LaggedStartMap(
                    FadeIn,VGcg,shift=LEFT,scale=1,lag_ratio=0.5
                    ),
                lag_ratio=0.2,
                run_time=1,
                )
...
class PictureShow2(PictureShow1):
    ...
    Anim_words = [["black", "white", "--v 5"],["黑","白", "--v 5"],["--seed 322570267"]]
...
class Picture1Show(Scene):
    ...
    def construct(self):
        ...
        self.play(
            FadeIn(ss_vg.center().shift(UP*0.236),shift=LEFT,scale=0.618,)
            )
        self.wait(1)
        self.play(
    ...
    def construct(self):
        ...
        self.play(
            LaggedStart(
                ...
                LaggedStartMap(
                    FadeIn,VGcg,shift=LEFT,scale=1,lag_ratio=1
                    ),
                lag_ratio=0.2,
                run_time=1,
                )
...
class PictureShow4(Scene):
    ...
    Coat = [
        ["手工合并图","AgPics/内衣3",],
        ["Midjourney生成图2","AgPics/内衣4",],
        ["Midjourney生成图1","AgPics/内衣5",],
        ]
    ar_buf = 0.5
    ar_scl = 0.96
    fshow = 2
    ...
    def construct(self):
        ...
        translate = Text("翻译",color=GREY,font = '阿里巴巴普惠体 2.0',font_size=40,)
        answer= TexText(self.Ans[0],color=YELLOW, font_size=self.f, alignment="\\raggedright")
        at = VGroup(translate,answer).arrange(DOWN,aligned_edge=LEFT,buff=0.25)
        ...
        self.play(FadeIn(vgjcopy,shift=RIGHT,))
        self.play(
            FadeIn(VGpqat[0][1][0]),
            ShowIncreasingSubsets(*VGpqat[0][1][1]),
            run_time=3,
        ...
        VGcg.arrange(RIGHT,buff=self.ar_buf,aligned_edge=UP).shift(UP*0.236).scale(self.ar_scl)
        self.play(
            LaggedStart(
                AnimationGroup(
                    FadeOut(vgjcopy),
                ...
                LaggedStartMap(
                    FadeIn,VGcg,shift=LEFT,scale=1,lag_ratio=self.lag_r
                    ),
                lag_ratio=0.2,
                run_time=1,
            )
